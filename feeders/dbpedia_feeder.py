...
def fetch_movie_info(url):
		...
		try:
			...
			if rdf_key_str in movieMapFromRdf.keys() and rdf_val_str != '*':
				...
				if key_in_result == ACTORS:
					if key_in_result in result.keys():
	...
	if LANGAUAGE in result.keys() and result[LANGAUAGE].startswith(DBPEDIA_PREFIX):
		result[LANGAUAGE] = fetch_name_from_dbpedia_page(result[LANGAUAGE])

	if COUNTRY in result.keys() and result[COUNTRY].startswith(DBPEDIA_PREFIX):
		...
		try:
			...
			for s, p, o in graph:
				try:
					...
					if rdf_key_str in personMapFromRdf.keys():
						actor_info[personMapFromRdf[rdf_key_str]] = rdf_val_str
				except:
					pass
			if BIRTH_PLACE in actor_info.keys() and actor_info[BIRTH_PLACE].startswith(DBPEDIA_PREFIX):
